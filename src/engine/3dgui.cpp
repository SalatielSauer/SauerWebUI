...
// SauerWUI
VAR(showcursor, 0, 1, 1);

void g3d_cursorpos(float &x, float &y)
{
    if(guis2d.length() || showcursor) { x = cursorx; y = cursory; }
    else x = y = 0.5f;
}
...
bool g3d_movecursor(int dx, int dy)
{
    ...
    if((!guis2d.length() || !hascursor) && !showcursor) return false;
    ...
    // SauerWUI
    cef_browser_move_mouse(int(cursorx * screenw), int(cursory * screenh));
...
bool g3d_windowhit(bool on, bool act)
{
    ...
    return showcursor || ((guis2d.length() && hascursor) || (windowhit && !windowhit->gui2d));
}
