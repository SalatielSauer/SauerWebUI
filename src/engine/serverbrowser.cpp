...
// SauerWUI
void json_escape(const char* src, vector<char>& dest)
{
    for (; *src; ++src)
    {
        switch (*src)
        {
        case '\\': dest.add('\\'); dest.add('\\'); break;
        case '"':  dest.add('\\'); dest.add('"'); break;
        case '\n': dest.add('\\'); dest.add('n'); break;
        case '\r': dest.add('\\'); dest.add('r'); break;
        case '\t': dest.add('\\'); dest.add('t'); break;
        default:   dest.add(*src); break;
        }
    }
}

// SauerWUI
void json_listservers(bool refresh)
{
    if (refresh) refreshservers();
    vector<char> buf;
    buf.add('[');
    ...
    loopv(servers)
    {
        ...
        if (s->resolved && s->ping != serverinfo::WAITING)
        {
            if (count > 0) buf.add(',');
            buf.put("{\"desc\": \"", 10);
            json_escape(s->sdesc, buf);
            buf.put("\", \"ip\": \"", 10);
            json_escape(s->name, buf);
            defformatstring(rest, "\", \"port\": %d, \"ping\": %d, \"map\": \"", s->port, s->ping);
            buf.put(rest, strlen(rest));
            json_escape(s->map, buf);
            defformatstring(tail, "\", \"numplayers\": %d}", s->numplayers);
            buf.put(tail, strlen(tail));
            ++count;
        }
    }
    buf.add(']');
...
ICOMMAND(json_listservers, "i", (int* refresh), json_listservers(*refresh != 0));
