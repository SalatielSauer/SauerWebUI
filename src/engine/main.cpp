...
// SauerWUI
bool cef_exit_called = false;
bool cef_shutdown_called = false;
...
void quit()                     // normal exit
{
    ...
    // SauerWUI - we need to safely close cef before killing the sdl process
    cef_cleanup();
    cef_exit_called = true;
...
void fatal(const char *s, ...)    // failure exit
{
    ...
    if(errors <= 2) // print up to one extra recursive error
    {
        ...
        if(errors <= 1) // avoid recursion
        {
            ...
            SDL_Quit();
            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "SauerWUI fatal error", msg, NULL);
        }
    }
...
void setupscreen()
{
    ...
    loopi(sizeof(configs)/sizeof(configs[0]))
    {
        ...
        screen = SDL_CreateWindow("Cube 2: Sauerbraten - SauerWUI", winx, winy, winw, winh, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | SDL_WINDOW_INPUT_FOCUS | SDL_WINDOW_MOUSE_FOCUS | flags);
        if(!screen) continue;
    ...
    // SauerWUI
    cef_browser_window_resize(screenw, screenh);
}
    ...
    while(pumpevents(events))
    {
        ...
        switch(event.type)
        {
            ...
            case SDL_MOUSEBUTTONUP:
                ...
                // SauerWUI
                cef_browser_click_mouse(event.button.button, (event.type == SDL_MOUSEBUTTONUP), 1);
...
// SauerWUI
void cef_javascript_result(const char* result) {
    if (strcmp(result, "undefined"))
    {
        conoutf(CON_ECHO, "%s", result);
    }
}

// SauerWUI
void cef_run_javascript_(char* code)
{
    cef_set_js_result_callback(cef_javascript_result);
    cef_run_javascript(code);
}
COMMANDN(javascript, cef_run_javascript_, "s");

// SauerWUI
void cef_trigger_page_load(void)
{
    execident("wui_onpageload");
}

// SauerWUI
void cef_execident(const char* code, cef_execident_result_callback_t result_cb, void* userdata)
{
    uint* compiled = compilecode(code);
    tagval result;
    executeret(compiled, result);
    const char* result_str = result.getstr();
    result_cb(result_str, userdata);
}
...
_SVAR(sauerwui_version, sauerwui_version, "01-11-2025", IDF_READONLY);
...
int main(int argc, char **argv)
{
...
#if defined(_WIN32)
    int exit_code = cef_execute_process(GetModuleHandle(NULL));
#else
    int exit_code = cef_execute_process(nullptr);
#endif
    if (exit_code >= 0)
    {
        // this is a cef subprocess; exit immediately.
        return exit_code;
    }
    ...
    // SauerWUI - cef initialization start
    logoutf("init: cef");
    ...
    execfile("data/wui/wui.cfg", false);

    cef_set_browser_loaded_callback(cef_trigger_page_load);
    cef_set_execident_callback(cef_execident);
    cef_set_input_active_callback(on_cef_input_active);
    cef_initialize(hwnd);
    cef_create_browser(hwnd, (char*)wui_default);
    // SauerWUI - cef initialization end
    ...
    for(;;)
    {
        ...
        if (cef_exit_called)
        {
            renderbackground("\f8saving & exiting..");
            if (!cef_shutdown_called)
            {
                cef_shutdown();
                cef_shutdown_called = true;
            }
            else
            {
