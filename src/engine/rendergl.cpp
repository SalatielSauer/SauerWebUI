...
// SauerWUI - cef cursor change events
static Texture* cursor_hand = nullptr;
static Texture* cursor_default = nullptr;
static Texture* cursor = nullptr;

void cef_cursor_change_handler(c_cef_cursor_type_t type)
{
    switch (type) {
    case _HAND:
        cursor = cursor_hand;
...
void drawcursor(int w, int h)
{
    ...
    if (windowhit)
    {
        ...
        if (!cursor)
        {
            cursor_hand = textureload("data/wui/wuipointer.png", 3, true);
            cursor_default = textureload("data/wui/wuicursor.png", 3, true);
            cursor = cursor_default;
            cef_set_cursor_change_callback(cef_cursor_change_handler);
        }

        float chsize = cursorsize * w / 900.0f;

        if (cursor->type & Texture::ALPHA) glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        ...
        hudshader->set();
        gle::color(vec(1, 1, 1)); // Cursor is always white
...
{
    // SauerWUI - always display cursor
    bool windowhit = g3d_windowhit(true, false);
...
void gl_drawhud()
{
    ...
    if (cef_pixel_buffer && cef_pixel_dirty) {
        ...
        if (!cefTexture)
        {
            glGenTextures(1, &cefTexture);
        };

        glBindTexture(GL_TEXTURE_2D, cefTexture);
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, cef_pixel_width, cef_pixel_height, 0,
            GL_BGRA, GL_UNSIGNED_BYTE, cef_pixel_buffer);

        cef_pixel_dirty = false;
    }
    ...
    if (cefTexture)
    {
        ...
        hudquad(0, 0, cef_pixel_width, cef_pixel_height, 0);
